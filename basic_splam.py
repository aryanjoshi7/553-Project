...
def cross_validate(X_full, y_full, penalty_fn, loss_fn, parameters: Parameters, k=20, random_state=1):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X_full)):
        ...
        for quantile in parameters.quantile_tau_list:
            ...
            return model
def report(curr_model):
    global qb_losses, mse_loss, mae_losses, mcpe_losses, quantiles, mse_losses
    print("----------------------------")
    for w in curr_model.weights:
        print(w)
    if curr_model is SPLAMAdditiveModel:
        print("linear weights: ", curr_model.linear_weights)
    ...
    qb_losses = defaultdict(list)
    mse_losses, mae_losses, mcpe_losses = [], [], []
...
y = y_1
y = y + 5 * np.random.normal(0, 0.3, size=n)
y_True = y + 5 * np.random.normal(0, 0.3, size=n)
y = y.reshape(-1, 1)
...
model_parameters = Parameters(quantile_tau_list=quantiles, lambda_=.012, scad_alpha=3.7, scad_lambda=1.5, splam_alpha=.3, formula=spline_formula)
splam_parameters = Parameters(quantile_tau_list=quantiles, lambda_=.025, scad_alpha=3.7, scad_lambda=1.5, splam_alpha=.3, formula=splam_formula, splam=True)

for i in range(5):
    model = cross_validate(features, labels, group_scad_penalty, quantile_loss, parameters=model_parameters, random_state=i, k=10)
report(model)
for i in range(5):
...
if basic:
    ...
    axs[0].scatter(features, labels, label='True $g_1$', linestyle='--')
    axs[0].plot(features, model_pred, label='Estimated $g_1$', color='red')
    ...
    axs[1].scatter(features, labels, label='True $g_2$', linestyle='--')
    axs[1].plot(features, splam_model_pred, label='Estimated $g_2$', color='red')
