import torch
from utils import *
from workHorse import getA


def rq_group_pen(x,y, tau=.5, groups=None, penalty="gLASSO", lamb=None, \
                 nlambda=100, eps=None, alg="huber", a=None, norm=2, \
                 group_pen_factor=None, tau_penalty_factor=None, scalex=True, \
                 coef_cutoff=1e-8, max_iter=5000,converge_eps=1e-4, gamma=None,\
                 lambda_discard=True, weights=None, *args):
    '''
    Params:
    x (torch.tensor):
    y (torch.tensor):
    ...
    '''
    if eps is None:
        eps = .05 if (x.shape[0] < x.shape[1]) else 0.01

    tau_penalty_factor = torch.ones(tau.numel()) \
        if(tau_penalty_factor is None) else tau_penalty_factor
    ...
    if(weights is not None):
        if(penalty=="ENet" | penalty=="Ridge"):
            stop("Cannot use weights with elastic net or ridge penalty. Can use\
                  it with lasso, though may be much slower than unweighted\
                  version.")
        if(penalty=="aLASSO"):
            #Warning
            print("WARN: Weights are ignored when getting initial (Ridge) \
                  estimates for adaptive Lasso")
    ...
    if(torch.sum(group_pen_factor)==0 | torch.sum(tau_penalty_factor)==0):
            stop("Cannot have zero for all entries of penalty factors. This \
                 would be an unpenalized model")
    ...
    if((weights is None) & penalty=="gAdLASSO"):
            #Warning
            print("WARN: Initial estimate for group adaptive lasso ignores the \
                  weights")
    ...
    if(norm == 1 & penalty == "gAdLASSO"):
        #Warning
        print("Group adapative lasso with 1 norm results in a lasso estimator where lambda weights are the same for each coefficient in a group. However, it does not force groupwise sparsity, there can be zero and non-zero coefficients within a group.")
    if(norm == 2 & alg != "huber"):
        stop("If setting norm = 2 then algorithm must be huber")
    if(penalty=="gAdLASSO" & alg != "huber"):
        #Warning
        print("huber algorithm used to derive ridge regression initial estimates for adaptive lasso. Second stage of algorithm used lp")
    ...
    if(lamb is None):
        ...
        lamMax = getLamMaxGroup(x,y,groups,tau,group_pen_factor,               \
                                penalty=penalty,scalex=scalex,                 \
                                tau_penalty_factor=tau_penalty_factor,         \
                                norm=norm,gamma=gamma,weights=weights)
        
        lamb = exp(seq(log(lamMax),log(eps*lamMax),length.out=nlambda))